...
public:
	...
	void TurnOnLight() { m_lightOn = true; }
	void TurnOffLight() { m_lightOn = false; }
	void IncreAmbient()
	{
		for (int i = 0; i < 3; ++i) {
			if (AmbientLight[i] < 1.0f) AmbientLight[i] += 0.02f;
		}
	}
	...
	{
		for (int i = 0; i < 3; ++i) {
			if (DiffuseLight[i] < 1.0f) DiffuseLight[i] += 0.03f;
		}
	}
	...
	{
		for (int i = 0; i < 3; ++i) {
			if (SpecularLight[i] < 1.0f) SpecularLight[i] += 0.04f;
		}
